...
def transform(path: Path) -> pd.DataFrame:
    ...
    # print(f"pre: missing passenger count: {data_frame['passenger_count'].isna().sum()}")
    # data_frame['passenger_count'].fillna(0, inplace=True)
    # print(f"post: missing passenger count: {data_frame['passenger_count'].isna().sum()}")
    return data_frame
...
@flow(log_prints=True)
def etl_parent_flow(
    months: list[int], year: int = 2019, color: str = "yellow"
):
    """Parent flow"""
    if not months:
        months = [2, 3]
